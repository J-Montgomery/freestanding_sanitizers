...
SRC_DIR = test/ubsan
TEST_SRC += $(SRC_DIR)/signed_addition_overflow.c $(SRC_DIR)/shift_overflow.c $(SRC_DIR)/signed_invalid_division.c $(SRC_DIR)/unsigned_invalid_division.c  $(SRC_DIR)/nonnull_return.c $(SRC_DIR)/invalid_load_bool.c
UBSAN_TESTS := $(patsubst %.c, %, $(TEST_SRC))

UNUSUAL_TEST_SRC += $(SRC_DIR)/test_death_override.c $(SRC_DIR)/float_invalid_division.c
UNUSUAL_UBSAN_TESTS := $(patsubst %.c, %, $(UNUSUAL_TEST_SRC))
UNUSUAL_FLAGS := -fno-sanitize-recover=all -fsanitize=float-divide-by-zero
